...
cols = db._columns(db.connect(), "accounts")
...
check("the dead lifetime column is gone from rounds",
      "lifetime" not in db._columns(db.connect(), "rounds"))
check("rounds gains a session to belong to", "session_id" in db._columns(db.connect(), "rounds"))
check("decisions gains what the chart wanted and where it came from",
      {"should", "source", "session_id"} <= db._columns(db.connect(), "decisions"))

kept = tuple(db.connect().execute(
    "SELECT (SELECT COUNT(*) FROM decisions), (SELECT COUNT(*) FROM rounds), "
    "(SELECT COUNT(*) FROM bets)").fetchone())
...
check("running the upgrade twice changes nothing",
      ...
      db.connect().execute("SELECT COUNT(*) FROM decisions").fetchone()[0] == 30)
...
left = tuple(db.connect().execute(
    "SELECT (SELECT COUNT(*) FROM sessions WHERE account_id=?), "
    "(SELECT COUNT(*) FROM ledger WHERE account_id=?), "
    "(SELECT COUNT(*) FROM quizzes WHERE account_id=?)", (acct, acct, acct)).fetchone())
